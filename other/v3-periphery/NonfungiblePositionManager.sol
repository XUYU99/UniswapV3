...
import "contracts/uniswap/v2-core/libraries/FixedPoint128.sol";
    ...
    SelfPermit
{
    ...
    /// @inheritdoc INonfungiblePositionManager
    /// @notice 为用户铸造新的流动性 NFT 头寸，并向池子添加流动性
    /// @param params 包含 token 对、价格区间、金额、最小值、接收者、期限等信息的结构体
    /// @return tokenId 流动性头寸的 NFT ID
        ...
        )
    {
        ...
        // console.log("NonfungiblePositionManager-mint()-222");
        // 给用户铸造一个新的 NFT，表示其流动性头寸
        _mint(params.recipient, (tokenId = _nextId++));
        // console.log("NonfungiblePositionManager-mint()-333");
        // 生成头寸的唯一 positionKey，用于查询池子中记录的头寸信息
        bytes32 positionKey = PositionKey.compute(
            address(this),
            params.tickLower,
            params.tickUpper
        );
        // console.log("NonfungiblePositionManager-mint()-444");
        // 获取池子中记录的该头寸的历史手续费状态（用于后续手续费增量计算）
        (
            ,
        ...
        // 将池子地址和 token/fx 组合映射到 poolId 并缓存
        ...
        // console.log("NonfungiblePositionManager-mint()-555");
        // 保存头寸数据到 _positions 映射中，以 tokenId 为索引
