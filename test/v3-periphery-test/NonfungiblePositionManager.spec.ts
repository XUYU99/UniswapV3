import { abi as IUniswapV3PoolABI } from "../../artifacts/contracts/uniswap/v3-core/interfaces/IUniswapV3Pool.sol/IUniswapV3Pool.json";
...
describe("NonfungiblePositionManager", () => {
  ...
  let nftAddress: string,
    ...
    tokens0Address: string,
    tokens1Address: string,
    weth9Address: string,
    routerAddress: string;

  const nftFixture = async () => {
    const [wallet, other] = await ethers.getSigners();
    const wallets = [wallet, other];
    ...
    tokens0Address = await tokens[0].getAddress();
    tokens1Address = await tokens[1].getAddress();
  ...
  //   expect(
  //     ((await ethers.provider.getCode(nftAddress)).length - 2) / 2
  //   ).to.matchSnapshot();
  // });
  it("bytecode size under limit", async () => {
    const bytecodeSize =
      ((await ethers.provider.getCode(nftAddress)).length - 2) / 2;
    console.log("Bytecode size:", bytecodeSize, "bytes");
    expect(bytecodeSize).to.be.lessThan(24576); // EVM 最大限制是 24KB
  });
  ...
  describe("#createAndInitializePoolIfNecessary", () => {
    ...
    it("could theoretically use eth via multicall", async () => {
      const [token0, token1] = sortedTokens(weth9Address, tokens0Address);
  ...
  describe("#mint", () => {
    it("fails if pool does not exist", async () => {
      await expect(
        nft.mint({
          token0: tokens0Address,
          token1: tokens1Address,
    ...
    it("creates a token", async () => {
      ...
      expect(token0).to.eq(tokens0Address);
      expect(token1).to.eq(tokens1Address);
  ...
  describe("#increaseLiquidity", () => {
    ...
    it("can be paid with ETH", async () => {
      const [token0, token1] = sortedTokens(tokens0Address, weth9Address);
  ...
  describe("#transferFrom", () => {
    ...
    it("removes existing approval", async () => {
      ...
      expect(await nft.getApproved(tokenId)).to.eq(AddressZero);
    });
