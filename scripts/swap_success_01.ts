...
export async function swapExactInputSingle() {
  ...
  console.log("第一次 200 next Tick, isInitialized:", nextInfo.toString());
  // 数据转成 sqrt
  ...
  console.log("第一次 200 tick sqcur:", sqcur.toString());
  console.log("第一次 sqnext:", sqnext.toString());
  // 计算 swap step
  ...
  console.log(
    "由于价格推进到了 SqTarget 因此本次swap 的amount0Step1 = Amount0Delta = ",
    Amount0Delta.toString()
  );
  ...
  console.log(
    "验证 第一次 swap 结果： swapInfo2 (return sqrtRatioNextX96,amountIn,amountOut,feeAmount) :",
    swapInfo.toString()
  );
  console.log("--------第二次 Swap Step---------");
  const liquidity2 = 199710149825819669480855n;
  const sqcur2 = await TickMathTest.getSqrtRatioAtTick(119);
  ...
  console.log("第二次 119 next Tick, isInitialized:", nextInfo2.toString());
  const sqnext2 = await TickMathTest.getSqrtRatioAtTick(nextInfo2.next);
  console.log("第二次 119 tick sqcur2:", sqcur2.toString());
  console.log("第二次 下一个tick 的 sqnext2:", sqnext2.toString());
  ...
  console.log(
    "amountRemaining2 扣掉手续费后的 amountSubFee:",
    amountSubFee2.toString()
  );
  const Amount0DeltaStep2 = await SqrtPriceMathTest.getAmount0Delta(
    sqcur2,
    sqnext2,
    liquidity2,
    roundUp
  ...
  console.log(
    "第二次 nextTick 到 curTick 的 amountIn TC Amount0Delta:",
    Amount0DeltaStep2.toString()
  );
  console.log(
    "由于价格没有推进到了 第二次SqTarget 因此本次swap 的amount0Step2要重新计算"
  );
  ...
  console.log("第二次 swap 的 价格更新 SqUpdate:", SqUpdate.toString());
  ...
  console.log("第二次 amount0Step2 (amountIn):", amount0Step2.toString());
  ...
  console.log(
    "验证 第二次 swap 结果：swapInfo2 (return sqrtRatioNextX96,amountIn,amountOut,feeAmount) :",
    swapInfo2.toString()
  );
